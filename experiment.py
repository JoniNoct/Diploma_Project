...
class LBPHModel:
        ...
        if os.path.exists(self.members_list_file):
            ...
            with open(self.members_list_file, "r") as file_reader:
                for line in file_reader:
                    member_id, member_name = line[:-1].split(',')
    ...
    def add_new_member(self, path_to_video: str = None):
        """
        ...
        detected_faces = self.face_detection(path_to_video=path_to_video)
    ...
    def face_detection(self, images_limit: int = 50, path_to_video: str = None):
        """
        The main process of collecting images of a new member's face.

        Args:
           images_limit(by default=50): amount of face images.
           path_to_video(by default=None): path to video file
    ...
    def recognize_experiment(self, test_images_limit: int = 1000):
        """
        The main process of recognizing member by images of a new member's face.
        If the model has not been trained before, the corresponding error will appear.
        And the recognizing process will be stopped.
        ...
        frame_counter = 0
        predictions = []
        real_label_id = get_key(self.members, 'Ilya')
        print(real_label_id)
        print(type(real_label_id))
        ...
        while True:
            ...
            if frame_counter == test_images_limit:
                break

        right_predictions = len([prediction for prediction in predictions if prediction == real_label_id])
        percentage = right_predictions/frame_counter * 100
        print(f"Frame limitation: {test_images_limit}")
        print(f"Amount of right predictions: {right_predictions}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if event.name == '1':
            ...
            print("You selected the 'Add test members' option\n")
            pyautogui.click()
            # Trying to predict who is in front of the camera
            main_model.add_new_member('video_samples/Basic interview left speaker.mp4')
            main_model.add_new_member('video_samples/Basic interview right speaker.mp4')
            main_model.add_new_member('video_samples/Product Manager Interview left speaker.mp4')
            ...
            print("You selected the 'Recognize experiment' option\n")
            pyautogui.click()
            # Trying to predict who is in front of the camera
            main_model.recognize_experiment()

        elif event.name == '5' or event.name == 'q' or event.name == 'esc':
